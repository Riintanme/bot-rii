                ...
                if (quotedMsg && quotedMsg.type == 'image') {
                    ...
                    const vidxvideo = await vidxvid.json()
                     ...
                     //   tobz.sendFileFromUrl(self, `https://sensorstechforum.com/wp-content/uploads/2019/07/xvideos-virus-image-sensorstechforum-com.jpg`, ``, captions, id)
                         tobz.sendFileFromUrl(self, mp4, `bokep.mp4`, `XVIDEOS Rii`, id).catch(() => tobz.reply(self, mess.error.Yt4, id))
                        await limitAdd(serial)
                        } catch (err){
                            console.log(err)
                        }
...
➸ *Wilayah:* +${region_name}`
     ...
     /*   case `${prefix}brainly`:
        ...
        case `${prefix}ownergroup`:
            ...
            break
            case `${prefix}tes` :
        let anjim = body.slice(5) 
        var tod = await tobz.getChatById(ids)
        if (!tod.isReadOnly) 
        await tobz.reply(self, `${anjim}` )
        break

      /*  case `${prefix}hidetag`: // FOR OWNER & ADMIN Rii
...
Text : ${txt1}`
        ...
        /*    argz = body.trim().split('')
            if (argz.length >= 3) {
                tobz.reply(self, mess.wait, id)*/
                const glitch1 = args[1]
                const glitch2 = args[2]
              ...
              //  if (glitch2.length > 8) return tobz.reply(self, '*Teks2 Terlalu Panjang!*\n_Maksimal 8 huruf!_', id) 
                tobz.sendFileFromUrl(self, `http://docs-jojo.herokuapp.com/api/ttlogo?text1=${glitch1}&text2=${glitch2}`)
                await limitAdd(serial)
    //        } else {
